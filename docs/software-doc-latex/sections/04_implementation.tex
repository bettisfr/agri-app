...
The app currently exposes two major control planes. The first is connectivity and device operation: discovery, Wi-Fi/AP switching, capture start/stop, one-shot dialog (RPi or ESP), and system actions (restart/reboot/poweroff). Auto-capture start supports source selection (\texttt{RPi}, \texttt{ESP}, or \texttt{Both}) with interval presets, and UI guardrails keep the flow unambiguous: start is disabled while any loop is active, while stop is enabled only when at least one loop is running. The second is gallery consumption: paginated list with thumbnails, image open in full-screen dialog, zoom/pan gestures, and previous/next swipe navigation.

A practical implementation detail is cache-first discovery. Last known hosts are persisted locally and probed before subnet scanning. This reduces connection latency in repeated sessions and limits unnecessary probing. During network mode switches (Wi-Fi $\leftrightarrow$ AP), the app now triggers an automatic discovery step after a short delay to recover quickly from expected subnet transitions. Another relevant detail is event-driven in-app logging: connection transitions, discovery outcomes, auto-loop lifecycle events, shot outcomes, and newly captured filenames are surfaced in a bottom log panel to improve field observability without requiring shell access.
