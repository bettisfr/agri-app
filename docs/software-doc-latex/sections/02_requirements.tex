...
Context metadata acquisition is also a functional requirement. The platform must persist best-effort GPS coordinates and BME280 environmental values (temperature, humidity, pressure) associated with new captures. Metadata persistence must be tolerant to partial sensor availability: when GPS is unavailable but BME280 is available (or vice versa), available values must still be stored without failing the capture.
