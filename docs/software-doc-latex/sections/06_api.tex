...
The Android app focuses on operational routes: discovery + health, status, capture one-shot (RPi/ESP), capture loop start/stop, gallery pagination with thumbnails, and system/network actions. The app intentionally does not introduce private backend APIs; it reuses the same versioned surface.
...
\begin{itemize}
  ...
  \item \textbf{Capture loop} endpoints control unattended periodic capture via backend-managed process state; \texttt{/start} accepts \texttt{interval\_seconds} in a bounded range.
\end{itemize}
