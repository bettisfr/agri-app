...
In addition to image sources, the edge tier integrates optional contextual sensors. A USB GPS receiver provides geospatial coordinates, while a BME280 on I2C provides environmental values. Both sensors are sampled with best-effort logic during capture workflows and are persisted as per-image sidecar metadata.
...
\begin{tikzpicture}[
    ...
    edge/.style={-{Latex[length=2mm]}, thick},
    meta/.style={-{Latex[length=2mm]}, thick, dashed}
]
\node[box] (scheduler) {Scheduler / API\\(RPi loop, ESP loop)};
\node[box, below=8mm of scheduler] (capture) {Capture workers\\RPi camera + ESP proxy};
\node[box, below left=11mm and 12mm of capture] (gps) {GPS};
\node[box, below right=11mm and 12mm of capture] (bme) {BME280};
\node[box, below=11mm of capture] (images) {images/};
\node[box, right=20mm of images] (metaStore) {metadata/};
\node[box, right=20mm of scheduler] (clients) {Web / Tablet\\Gallery + Labeler};

\draw[edge] (scheduler) -- (capture);
\draw[edge] (capture) -- (images);
\draw[meta] (gps) -- (metaStore);
\draw[meta] (bme) -- (metaStore);
\draw[meta] (capture.east) -- ++(7mm,0) |- (metaStore.west);
\draw[edge] (clients.west) -- node[above, font=\scriptsize]{HTTP API} (scheduler.east);
\draw[edge] (clients.west) -- ++(-8mm,0) |- node[right, font=\scriptsize]{list/view} (images.east);
\draw[edge] (clients.west) -- ++(-6mm,0) |- node[right, font=\scriptsize]{context} (metaStore.east);
\end{tikzpicture}
\caption{Runtime orchestration: capture loops, sensor enrichment, and client data paths.}
\label{fig:agriapp-runtime-orchestration}
\end{figure}
...
Metadata sidecars follow the same lifecycle constraints: when an image is removed, associated metadata artifacts are removed as well. This preserves consistency between binary image content and contextual data.
