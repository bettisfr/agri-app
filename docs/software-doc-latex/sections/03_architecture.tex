...
\node[box, below=8mm of web] (tablet) {Android App\\(AgriApp Control)};
...
Client responsibilities are intentionally limited. The web gallery and the tablet app focus on orchestration, visualization, and interaction, while state authority remains in the backend. On Android, this boundary is reflected in the UI split between a control dashboard (operations/discovery) and a dedicated gallery activity (dataset browsing). This separation avoids duplicated business rules in frontend code and preserves consistency when multiple clients operate on the same dataset.
