...
In the current hardware baseline, the operational GPS module is the previously used GT-U7, restored after comparative trials with a NEO-6M unit that showed unstable behavior in the tested field setup.
For ESP32-CAM image capture validation, the operational baseline currently uses the OV3660 path with stable backend preset (\texttt{framesize=qxga}, \texttt{quality=10}). OV5640 trials were performed but excluded from baseline operation due to recurrent optical artifacts in field shots.
...
In current Studio workflow, an explicit \texttt{Clear all labels} action is part of validation because segmentation campaigns may generate tens of boxes per image; this operation must clear in-memory labels deterministically and persist only after explicit save.

\subsection{SegGPT Cross-Node Comparative Validation}
For paper-oriented reproducibility, SegGPT is validated with a dedicated cross-node benchmark script (\texttt{scripts/seggpt\_compare\_benchmark.py}). The benchmark executes the same model path on:
\begin{itemize}
  \item Studio node (local run),
  \item Raspberry Pi node (remote run via SSH).
\end{itemize}

For each test image, the benchmark records:
\begin{itemize}
  \item local and remote elapsed time,
  \item box-level equivalence,
  \item optional mask hash equivalence (when remote masks are downloaded),
  \item per-node raw JSON payloads.
\end{itemize}

Artifacts are exported under a timestamped run folder with a summary CSV and detailed JSON traces. This provides a stable evidence trail for comparative sections in the manuscript while keeping runtime UI concerns separated from evaluation workflows.
...
\subsection{Reproducibility Artifacts for Publication}
Comparative experiments are exported as filesystem artifacts, not only terminal logs. The benchmark workflow stores:
\begin{itemize}
  \item \texttt{summary.csv} (one row per image with local/remote times and match flags),
  \item \texttt{json/local\_<stem>.json} and \texttt{json/remote\_<stem>.json},
  \item local masks and optional downloaded remote masks for hash-level checks.
\end{itemize}

This packaging enables downstream auditability: a reviewer can re-open one run folder and verify timings, statuses, and semantic equivalence without replaying the whole session.
