...
The fourth objective is network resilience. The same software instance should remain usable across multiple Wi-Fi environments and in AP rescue mode. Discovery and client behavior must be fast enough for field use, privileging cache-first strategies and minimizing unnecessary scans.

Finally, the fifth objective is maintainable evolution. The platform should be extendable without repeated rewrites by enforcing modular frontend organization, clear backend boundaries, and versioned endpoint semantics.
