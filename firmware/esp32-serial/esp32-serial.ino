...
#include "../esp32-cam/board_config.h"
...
static bool init_camera() {
  ...
  if (s != nullptr) {
    ...
    // Neutral sensor profile: keep natural orientation and colors.
    s->set_hmirror(s, 0);
    s->set_vflip(s, 1);
...
void loop() {
  // Optional on-demand capture by sending 'c' over serial.
  while (Serial.available() > 0) {
    ...
    if (ch == 'c' || ch == 'C') {
      ...
      // Parse line commands such as: set hmirror=0
      String line;
      line += (char)ch;
